...
export const teamSteps = [
	...
	},
	{
		...
		detail:
			"Transforme le besoin produit en spec technique pour l'IA. Un développeur reste un développeur — il ajoute à son métier une compétence de cadrage : le prompt engineering appliqué au projet, dans son vocabulaire.",
	},
	{
		role: 'Dev + IA',
	...
	{
		role: 'Reviewer (IA différente)',
		action: 'Audit cross-model via lyt review',
		detail:
			"Une session IA fraîche — idéalement un vendor différent de l'implémenteur — lance `lyt review ISS-XXXX` sur la branche. Elle lit l'issue, le diff, les rules, la skill code-review, et retourne un verdict GO ou NO_GO. NO_GO renvoie l'issue en 3-in-progress avec une liste de corrections concrète ; GO la laisse en 4-review pour validation humaine. L'indépendance des biais est tout l'intérêt : un modèle qui audite son propre code partage les angles morts qui ont causé l'erreur.",
	},
	{
		role: 'Dev',
		action: 'Ouvre une PR',
	...
	},
	{
		...
		action: 'Review humaine',
		detail:
			"Passe humaine finale par-dessus l'audit cross-model. Vérifie que les rules et skills sont respectés. Ajoute à memory ce qui doit être appris pour les issues suivantes.",
	},
	{
		role: 'CI/CD',
